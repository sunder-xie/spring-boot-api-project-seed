...
//				.compile("");
	...
	public static boolean matchZipCodeRule(String msg) {
		// 邮编必须是6位数字
		String regex = "[\\d]{6}";
		if (com.cpic.caf.core.utils.StringUtil.isNotBlank(msg)) {
	...
	public static boolean matchVin(String vin) {
		...
		if (com.cpic.caf.core.utils.StringUtil.isNotBlank(vin)) {
	...
	public static boolean matchLicensePlateNo(String licensePlateNo) {
		...
		if (com.cpic.caf.core.utils.StringUtil.isNotBlank(licensePlateNo)) {
	...
	public static boolean matchIdCardNo(String idCardNo)
	{
		...
		if (com.cpic.caf.core.utils.StringUtil.isNotBlank(idCardNo)) {
	...
	public static boolean validateVin(String vin) {
		...
		if (com.cpic.caf.core.utils.StringUtil.isNotBlank(vin)) {
			vin=vin.trim();
			if(com.cpic.caf.core.utils.StringUtil.isBlank(vin)){
				return false;
			}
			String regex = "^[A-HJ-NPR-Z0-9]{1,17}$";
	...
	public static boolean validateAccount(String accountNo) {
		if (com.cpic.caf.core.utils.StringUtil.isBlank(accountNo) || accountNo.length() < 10) {
			return false;
		}
		...
		for (int i = len - 1; i >= 0; i--) { // 倒序赋值数值字串转成数值数组
			...
			if (com.cpic.caf.core.utils.StringUtil.isBlank(curr)) {//为空时判断为无效账号
				return false;
			}
